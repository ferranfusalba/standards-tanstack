...
describe("createFuzzyFilter", () => {
	...
	it("returns false when value does not match", () => {
		const meta = makeMeta();
		const result = fuzzyFilter(makeRow("Japan"), "code", "xyz", (m) => {
			Object.assign(meta, m);
		});
	...
	it("adds itemRank to meta", () => {
		...
		fuzzyFilter(makeRow("Euro"), "code", "eur", (m: FilterMeta) => {
...
describe("fuzzyFilterAcronym (ACRONYM threshold)", () => {
	it("matches acronyms", () => {
		const result = fuzzyFilterAcronym(
			makeRow("United States"),
			"col",
			"us",
			() => {},
...
describe("rankItem integration", () => {
	...
	it("threshold filters out low-quality matches", () => {
		...
		const withContains = rankItem("United States", "us", {
			threshold: rankings.CONTAINS,
		});
		const withAcronym = rankItem("United States", "us", {
