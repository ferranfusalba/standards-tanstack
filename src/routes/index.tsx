...
function App() {
  const pages = [
    ...
    {
      icon: <Languages className="w-12 h-12 text-cyan-400" />,
      title: 'Languages',
      description:
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {pages.map((page, index) => (
            <Link
              key={index}
              to={page.path}
              className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6 hover:border-cyan-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-cyan-500/10 block"
            >
